...
export const KNOWLEDGE_FOCUS: Record<string, string> = {
...
export function focusForTags(tags: string[]): string {
  const unique = Array.from(new Set(tags.length ? tags : ['FDS']));
  return unique.map((tag) => KNOWLEDGE_FOCUS[tag] ?? KNOWLEDGE_FOCUS.FDS).join(' ');
}

function answerPrefix(answer: string[]): string {
  return `选 ${answer.join(', ')}`;
}

function optionText(answer: string[] | undefined, options: QuestionOption[] | undefined): string {
...
function specificExplanation(
  ...
  kind: string,
  context: ExplanationContext | string | undefined
): string | undefined {
...
export function explainAnswer(
  ...
  tags: string[],
  kind: string,
  context?: ExplanationContext | string
): string {
  const specific = specificExplanation(answer, kind, context);
...
export function summarizeKnowledgePoints(questions: Question[]): KnowledgeSummary[] {
  const counts = new Map<string, number>();
  for (const question of questions) {
    for (const tag of new Set(question.tags.length ? question.tags : ['FDS'])) {
  ...
  return Array.from(counts.entries())
    .map(([tag, count]) => ({
      ...
      focus: KNOWLEDGE_FOCUS[tag] ?? KNOWLEDGE_FOCUS.FDS
    }))
